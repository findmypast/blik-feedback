...
def dashboard(request):
    ...
    # Count the top-level items presented on the dashboard, rather than every
    # underlying per-person ReviewCycle generated by a campaign. One
    # organisation cycle or campaign is therefore one active cycle.
    active_cycles = (
        len(organizational_cycle_cards)
        + len(active_campaigns)
        + len(active_cycles_data)
    )
